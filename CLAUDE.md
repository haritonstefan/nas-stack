...
- **Spec is vendored at `reference/jellyfin-openapi.json`** (recipes in `reference/README.md`). Query it with `jq` — **never `Read` or WebFetch it**: 1.9MB on one line, truncates alphabetically before `/Startup`. Its `info.version: 12.0.0` is meaningless — there is no Jellyfin 12; the line is 10.x.
